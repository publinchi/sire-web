...
export class AppMenuComponent implements OnInit {
    ...
    ngOnInit() {
        this.model = [
            {label: 'Home', icon: 'home', routerLink: ['/'], component: this.dictionary['']},
            {label: 'Pedidos', icon: 'shopping_cart', routerLink: ['/pedido'], component: this.dictionary['pedido']},
            {
        ...
        <ng-template ngFor let-child let-i="index" [ngForOf]="(root ? item : item.items)">
            <li [ngClass]="{'active-menuitem': isActive(i)}" *ngIf="child.visible === false ? false : true">
                ...
                <a (click)="itemClick($event,child,i)" class="ripplelink" *ngIf="child.routerLink"
                    routerLinkActive="active-menuitem-routerlink" [attr.tabindex]="!visible ? '-1' : null" [attr.target]="child.target">
                    <i class="material-icons">{{child.icon}}</i>
                    <span>{{child.label}}</span>
                    <i class="material-icons" *ngIf="child.items">keyboard_arrow_down</i>
                </a>
...
export class AppSubMenu {
    ...
    constructor( @Inject(forwardRef(() => AppComponent)) public app: AppComponent, public location: Location, public appCtrl: App) {}
    ...
    openPage(page) {
        ...
        if (page.component != null)
            setTimeout(() => {this.appCtrl.getRootNav().push(page.component)}, 500);
    }
}